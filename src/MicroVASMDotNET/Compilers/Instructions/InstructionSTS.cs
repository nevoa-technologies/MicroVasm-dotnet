...
{
    class InstructionSTS : IInstruction, IInstruction<ISpecRegisters>, IInstruction<ISpecTypes>, IInstruction<ISpecReferences>
    {
        public string Name => "STS";
        ...
        public byte[] Generate(ICompiler compiler, VASMInstructionData instruction)
        {
            ...
            {
                compiler.ThrowError(instruction, "STS instruction must have 3 parameters. " + instruction.Parameters.Count + " parameters found.");
                return new byte[] { 0 };
            }
            ...
            {
                compiler.ThrowError(instruction, "STS instruction must have a register in the first parameter. The first parameter is not a register.");
                return new byte[] { 0 };
            }

            if (!types.GetTypeSize(instruction.Parameters[2], out size, out isUnsigned))
            {
                compiler.ThrowError(instruction, "STS instruction must have a type in the third parameter. The third parameter is not a type or a size.");
                return new byte[] { 0 };
            }

            if (size > compiler.MaxTypeSize)
            {
                compiler.ThrowError(instruction, "STS instruction must have a type with a size not bigger than " + compiler.MaxTypeSize + ".");
                return new byte[] { 0 };
            }
            ...
            if (value == null)
            {
                ...
                {
                    compiler.ThrowError(instruction, "STS instruction must have a definition name or stack address.");
